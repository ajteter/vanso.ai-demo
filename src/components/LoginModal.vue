    ...
    <Transition name="login-overlay">
      ...
      >
        ...
        <Transition name="login-modal">
          <div class="bg-zinc-950 border border-white/10 rounded-3xl shadow-[0_30px_60px_rgba(0,0,0,0.6)] w-105 max-w-[90vw] p-10 flex flex-col items-center gap-[28px] relative">
            
            <!-- Close Button -->
            <button 
              class="absolute top-4 right-[16px] size-8 flex items-center justify-center rounded-full cursor-pointer transition-colors duration-300 hover:bg-white/5"
              @click="$emit('close')"
            >
              <svg class="size-3 text-zinc-400" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M1 1L11 11M1 11L11 1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
              </svg>
            </button>

            <!-- Vanso Logo -->
            <div class="size-16 rounded-full overflow-hidden shadow-[0_0_30px_rgba(245,50,138,0.2)]">
            ...
            <button 
              class="flex items-center justify-center gap-3 w-full bg-white hover:bg-gray-100 rounded-full py-[14px] px-6 transition-colors duration-300 cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.15)]"
            ...
            >
              ...
              <span class="font-semibold text-[15px] text-zinc-950 leading-[20px]">Sign in with Google</span>
            </button>

            <!-- Privacy Notice -->
            <p class="text-xs text-zinc-500 text-center leading-[18px] max-w-75">
