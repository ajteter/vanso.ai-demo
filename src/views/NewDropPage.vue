...
<template>
  <div class="flex flex-col gap-8 items-start max-w-250 pb-[128px] pt-[40px] px-8 w-full">
    ...
    <div class="flex items-start justify-between w-full">
      ...
      <!-- Close Button -->
      <button 
        class="backdrop-blur-lg bg-white/5 border border-white/10 flex items-center justify-center rounded-full size-10 cursor-pointer transition-colors duration-300 hover:bg-white/10"
        @click="goBack"
      >
    ...
    <div 
      v-if="!audioFileName"
      class="backdrop-blur-lg border-2 border-dashed flex flex-col h-26 items-center justify-center rounded-xl w-full cursor-pointer transition-all duration-300"
      :class="isDragging 
        ? 'bg-[rgba(84,227,213,0.08)] border-vanso-cyan' 
        : 'bg-white/5 border-white/10 hover:border-[rgba(255,255,255,0.2)]'"
    ...
    <div 
      v-else
      class="backdrop-blur-lg bg-[rgba(84,227,213,0.03)] border border-[rgba(84,227,213,0.3)] flex items-center rounded-xl w-full px-6 py-5"
    >
      <input ref="audioInput" type="file" accept="audio/*" class="hidden" @change="handleFileSelect" />
      ...
      <!-- Delete/trash button -->
      <button 
        class="size-9 flex items-center justify-center rounded-lg cursor-pointer transition-colors duration-300 hover:bg-[rgba(239,68,68,0.1)]"
    ...
    <div class="flex flex-col gap-8 items-start pb-[16px] w-full">
      ...
      <div class="flex gap-6 items-start w-full">
        ...
        <div class="flex-1 flex flex-col gap-2">
          ...
          <div class="backdrop-blur-md bg-white/5 border border-white/10 rounded-lg overflow-hidden shadow-[inset_0px_2px_4px_1px_rgba(0,0,0,0.05)]">
            <input 
              v-model="title"
              type="text" 
        ...
        <div class="flex-1 flex flex-col gap-2">
          ...
          <div class="relative backdrop-blur-md bg-white/5 border border-white/10 rounded-lg overflow-hidden shadow-[inset_0px_2px_4px_0px_rgba(0,0,0,0.05)]">
            <select 
              v-model="language"
      ...
      <div class="flex gap-8 items-start w-full">
        ...
        <div class="flex flex-col gap-2 w-50 shrink-0">
          ...
          <div 
            class="backdrop-blur-md bg-white/5 border border-white/10 rounded-lg overflow-hidden size-50 flex flex-col items-center justify-center cursor-pointer transition-colors duration-300 hover:bg-[rgba(255,255,255,0.08)] shadow-[inset_0px_2px_4px_1px_rgba(0,0,0,0.05)] relative"
        ...
        <div class="flex-1 flex flex-col gap-2 self-stretch">
          ...
          <div class="backdrop-blur-md bg-white/5 border border-white/10 rounded-lg overflow-hidden flex-1 min-h-50 shadow-[inset_0px_2px_4px_1px_rgba(0,0,0,0.05)]">
            <textarea 
              v-model="description"
    ...
    <!-- Action Buttons -->
    <div class="border-t border-white/10 flex items-center justify-between pt-[25px] w-full">
      ...
      <label class="flex items-start gap-2.5 cursor-pointer max-w-150 shrink">
        <div class="relative flex items-center mt-[2px]">
          <input 
            v-model="confirmed" 
            type="checkbox" 
            class="peer appearance-none size-4 border border-[rgba(255,255,255,0.2)] bg-white/5 rounded-[4px] checked:bg-vanso-cyan checked:border-vanso-cyan cursor-pointer transition-colors"
          />
      ...
      <div class="flex gap-4 items-center shrink-0">
        <button class="border border-transparent rounded-full px-[25px] py-[13px] cursor-pointer transition-colors duration-300 hover:bg-white/5">
